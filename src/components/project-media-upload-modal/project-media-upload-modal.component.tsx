...
import { Box, Typography, Button, Tab, Tabs, Dialog, DialogTitle, IconButton, Stack, ImageList, ImageListItem, ImageListItemBar } from '@mui/material';
...
import { FilesTypes, UploadModalProps } from './project-media-upload-modal.type';
...
export default function UploadModal({ open, onClose }: UploadModalProps) {
    ...
    // const calculateFileIndex = (tabIndex: number, fileIndex: number): number => {
    //     switch (tabIndex) {
    //         case 0:
    //             return (imagesFiles?.length || 0) + fileIndex + 1;
    //         case 1:
    //             return (imagesFiles?.length || 0) + (videoFiles?.length || 0) + fileIndex + 1;
    //         case 2:
    //             return (imagesFiles?.length || 0) + (videoFiles?.length || 0) + (audioFiles?.length || 0) + fileIndex + 1;
    //         default:
    //             return fileIndex + 1;
    //     }
    // };
    ...
    // const handleConceptNameChange = (index: number, value: string, fileType: string) => {
    //     switch (fileType) {
    //         case 'image':
    //             setImagesFiles(files => files.map((file, i) => i === index ? { ...file, concept_name: value } : file));
    //             break;
    //         case 'video':
    //             setVideoFiles(files => files.map((file, i) => i === index ? { ...file, concept_name: value } : file));
    //             break;
    //         case 'audio':
    //             setAudioFiles(files => files.map((file, i) => i === index ? { ...file, concept_name: value } : file));
    //             break;
    //     }
    // };
    ...
    console.log(imagesFiles, videoFiles, audioFiles, "finalDatafinalDatafinalData")
                ...
                <Box sx={{ bgcolor: 'background.paper', padding: '1rem 2rem', height: '100%', display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", position: 'sticky', top: 0, zIndex: 1, bgcolor: 'background.paper' }}>
                        ...
                        <Box style={{ display: "flex" }}>
                            ...
                            {(isUploadDisabled > 0) &&
                                <Button variant='contained' disabled={!(isUploadDisabled > 0)} onClick={() => {
                                    uploadFiles()
                                    onClose()
                                }}>Save {isUploadDisabled ? `(${isUploadDisabled})` : ''}</Button>
                            }
                        </Box>
                    </Box>
