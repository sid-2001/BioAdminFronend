...
interface VisualizationCreateUpdateProps {
  PutDataVisualizations(visualization_id: number, payload: any): Promise<void>;
  visualizationData: any;
  PostQuestion: (payload: any) => Promise<void>;
  DeleteQuestion: (payload: any) => Promise<void>;
}
...
const VisualizationsUpdateComponent: React.FC<VisualizationCreateUpdateProps> = ({
  PutDataVisualizations, visualizationData, PostQuestion, DeleteQuestion
}) => {
    ...
    if (projectId && surveyId && selectedOption) {
      try {
        ...
        const processBannerList = data && data?.banner_config?.length > 0 && data?.banner_config?.map((item: { question_value: any; question_text: any; question_code: any; question_label: any; grouped_question_code: any; }) => ({
          question_value: item?.question_label,
  ...
  // console.log(tabulationData, "tabulationData", bannerPointsList)
  ...
  const handleSubmit = async (e: any) => {
    ...
    const questionIds = visualizationData?.question_config?.map((item: { question_id: any; }) => item.question_id);

    const newQues = question_config?.filter(item => !questionIds.includes(item.question_id))

    const updateQues = question_config?.filter(item => questionIds.includes(item.question_id))

    const missingQuestionIds = questionIds.filter((id: any) => !dataToEdit.some(item => item.question_id === id));

    const finalPayload = {
      name: jobName,
      version: '0.1-beta',
      tabulation_id: tabulation,
      question_config: updateQues
    };
    // console.log(questionIds, "questionIdsquestionIds", newQues, updateQues, missingQuestionIds)
    // console.log(finalPayload, "finalPayloadfinalPayload")
    await PutDataVisualizations(Number(visualizationId), finalPayload);
    if (newQues && newQues.length > 0) {
      for (const question of newQues) {
        await PostQuestion(question);
      }
    }
    if (missingQuestionIds && missingQuestionIds?.length > 0) {
      await DeleteQuestion(missingQuestionIds)
    }
  };
  ...
  const AddQuestionToEdit = () => {
    ...
    if (matchedQuestions) {
      ...
      // const newQuestions = transformedData.filter((newQuestion: { question_value: any; }) =>
      //   !dataToEdit.some(existingQuestion =>
      //     existingQuestion.question_value === newQuestion.question_value
      //   )
      // );

      // if (newQuestions.length > 0) {
      //   setDataToEdit(prevData => [...prevData, ...newQuestions]);
      // }
      console.log(selectQuestionList, questionList, selectedQuestions, matchedQuestions, "matchedQuestionsmatchedQuestionsmatchedQuestions")
  ...
  const handleRiskInputChangeUseEffect = async (value: number) => {
    setLoading(true)
  ...
  useEffect(() => {
    if (projectId && surveyId && visualizationId) {
      getAllTabulationData();
      GetProjectBannerALLQuestions();
    }
  }, [projectId, surveyId, visualizationId]);
  ...
  useEffect(() => {
    const editData = visualizationData && visualizationData?.question_config?.map((item: { question_type: any; chart_type: any; question_variable_id: any; question_id: any; }) => ({
      ...item,
      question_type_name: item?.question_type,
      chart_type: item?.chart_type,
      question_id: item?.question_id,
      question_value: item?.question_id,
      question_type_id: questionList?.find((nitem: { question_id: any; }) => nitem?.question_id == item?.question_id)?.question_type_id,
      answers: questionList?.find((nitem: { question_id: any; }) => nitem?.question_id == item?.question_id)?.answers,
      question_variable_id: questionList?.find((nitem: { question_id: any; }) => nitem?.question_id == item?.question_id)?.question_variable_id,
      // question_type_name: questionList?.find((nitem: { question_id: any; }) => nitem?.question_id == item?.question_id)?.question_type_name,
    }))

    // console.log(editData, "editDataeditData", questionList)
    setDataToEdit(editData)

    const questionIds = visualizationData?.question_config?.map((item: { question_id: any; }) => item.question_id);
    setSelectedQuestions(questionIds);

    setJobName(visualizationData?.name)
    setTabulation(visualizationData?.tabulation_id)

    handleRiskInputChangeUseEffect(visualizationData?.tabulation_id)


  }, [visualizationData, questionList])

  // console.log(visualizationData, "visualizationDatavisualizationData")

  // console.log(selectedQuestions, "selectedQuestionsselectedQuestions", selectBanners, dataToEdit)

  // console.log(tabulation, "visualizationIdvisualizationIdvisualizationIdvisualizationId")

  // console.log(selectQuestionList, "selectQuestionListselectQuestionList", questionList?.find((nitem: { question_id: any; }) => nitem?.question_id == 'IB')?.question_variable_id)
...
export default VisualizationsUpdateComponent;
