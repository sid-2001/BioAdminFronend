import { Box, Button, Checkbox, FormControlLabel, Grid, IconButton, ListItemIcon, MenuItem, MenuList, Typography } from '@mui/material'
...
const PipelinesCreateUpdate: React.FC<PipelinesCreateUpdateProps> = ({ PostDataTabulation }) => {
  ...
  const [otaCheck, setOtaCheck] = useState(false)
  ...
  const [decimal, setDecimal] = useState<any>(2)
  ...
  // console.log(allVisualizations, "allVisualizationsallVisualizations", allTabulations, selectedVisData)
  ...
  const handleRiskInputChange = async (event: { target: { value: any } }) => {
    ...
    const selectedOption = allTabulations.find((option) => option.value == selectedValue)
    // console.log(selectedValue, "selectedValueselectedValue", selectedOption);

    if (projectId && surveyId && selectedOption) {
      try {
        const data = await projectDataService.GetTabulationByProjectId(Number(projectId), Number(surveyId), selectedOption.object_uid)
  ...
  async function GetProjectBannerALLQuestions() {
    ...
    try {
      const data = await service.getProjectBannerALLQuestions(Number(projectId), Number(surveyId))
      if (data?.length > 0) {
        ...
        const filteredData = data.filter((item) => !excludedTypes.includes(item.question_type_name) && item.question_variable_type !== 'HIDDEN')
        ...
        const chartFilterData = data.filter((item: any) => item.is_chartable_question === true && item.question_variable_type !== 'HIDDEN')
        ...
        if (finalChartDAta && finalChartDAta) {
          const serviceNames = finalChartDAta.map(
            // @ts-ignore
            (item: { question_value: any; question_text: any; question_variable_id: string }) => ({
      ...
      enqueueSnackbar(<Typography variant="body1">Fetching questions failed</Typography>, {
        variant: 'error',
      })
    } finally {
      setLoading(false)
  ...
  const handleDataChartsConfigChange = (configType: any, fieldName: any, newValue: any) => {
    setDataChartsConfig((prevConfig) => {
      ...
      const configIndex = updatedConfig && updatedConfig.config.findIndex((c) => c.type === configType)
      if (configIndex !== -1) {
        if (fieldName === 'enabled') {
          updatedConfig.config[configIndex].enabled = newValue
        } else {
          const fieldIndex = updatedConfig.config[configIndex].fields.findIndex((f: any) => f.name === fieldName)
          if (fieldIndex !== -1) {
            // @ts-ignore
            updatedConfig.config[configIndex].fields[fieldIndex].value = newValue
          }
        }
      }
    ...
    setDataCleaningConfig((prevConfig) => {
      ...
      if (configIndex !== -1) {
        if (fieldName === 'enabled') {
          ...
          const fieldIndex = updatedConfig.config[configIndex].fields.findIndex((f) => f.name === fieldName)
          if (fieldIndex !== -1) {
            updatedConfig.config[configIndex].fields[fieldIndex].value = newValue
          }
        }
  ...
  const [dataValidationConfig, setDataValidationConfig] = useState<DataValidationConfig>(initialValidation)
  ...
  const handleDataValidationFieldChange = (fieldName: string, value: boolean) => {
    setDataValidationConfig((prevConfig) => ({
      ...prevConfig,
      config:
        prevConfig &&
  ...
  const [dataExportConfig, setDataExportConfig] = useState<DataValidationConfig>(initialExport)
  ...
  const [dataInsightsConfig, setDataInsightsConfig] = useState<DataValidationConfig>(initialInsights)
  ...
  const [dataOtaConfig, setDataOtaConfig] = useState<DataValidationConfig>(initialOta)
  ...
  const [dataPPTConfig, setDataPPTConfig] = useState<DataValidationConfig>(initialPPT)
  ...
  // console.log(
  //   dataInsightsConfig,
  //   "dataInsightsConfigdataInsightsConfigdataInsightsConfig",
  //   dataOtaConfig
  // );
  ...
  // console.log(dataPPTConfig, "dataPPTConfigdataPPTConfig", dataInsightsConfig)
  ...
  const mapSelectedMarketIdsToQuestions = () => {
    return selectedMarketIds.map((id) => {
      const foundItem: any = marketList.find((item: any) => item.value === id)
      return {
    ...
    if (tabulationData) {
      configPayload.push({
        ...
        config_payload: {
          ...
          risk_level_perc: tabulationData?.confidence_level,
  ...
  // console.log(tabulationData, "tabulationDatatabulationData");
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Typography variant="h3">{/* {(cardName as any)?.pipeline_name} */}</Typography>
        </Box>
        <Box style={{ padding: '0rem 2rem 2rem 2rem' }}>
          ...
          <Box style={{ padding: '1rem 0rem 2rem 0rem' }}>
            <form onSubmit={handleSubmit}>
              ...
              <Grid container spacing={2} style={{ marginTop: '1rem', height: '100%', gap: '1rem' }}>
                <Grid
                  item
                  xs={3.8}
                  style={
                ...
                >
                  ...
                  <MenuList>
                    ...
                    >
                      ...
                      >
                        <SettingsOutlinedIcon
                          color="primary"
                          style={{
                            // opacity: dataFilterConfig?.config?.length > 0 ? 1 : 0.3,
                          }}
                        />
                      </div>
                    </MenuItem>
                    ...
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'Data_Cleaning' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('Data_Cleaning')}
                    >
                      <ListItemIcon>
                        <FormControlLabel
                          control={<IOSSwitch sx={{ m: 1 }} checked={cleainingCheck} onChange={() => setCleaningCheck(!cleainingCheck)} />}
                          label=""
                        />
                      </ListItemIcon>
                      Data Cleaning
                    ...
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'Data_Validation' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('Data_Validation')}
                    >
                      <ListItemIcon>
                        <FormControlLabel
                          control={<IOSSwitch sx={{ m: 1 }} onChange={() => setValidationCheck(!validationCheck)} checked={validationCheck} />}
                          label=""
                        />
                      </ListItemIcon>
                      Data Validation
                      ...
                      >
                        <SettingsOutlinedIcon
                          color="primary"
                          style={{
                            opacity: dataValidationConfig?.config?.length > 0 ? 1 : 0.3,
                          }}
                        />
                      </div>
                    </MenuItem>
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'Data_Tabulation' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('Data_Tabulation')}
                    >
                      <ListItemIcon>
                        <FormControlLabel
                          control={<IOSSwitch sx={{ m: 1 }} onChange={() => setTabulationCheck(!tabulationCheck)} checked={tabulationCheck} />}
                          label=""
                        />
                      </ListItemIcon>
                      Data Tabulation
                      ...
                      >
                        <SettingsOutlinedIcon
                          color="primary"
                          style={{
                            opacity: tabulationData?.banner_config?.length > 0 && (decimal || decimal == 0) && minimumSampleSize ? 1 : 0.3,
                          }}
                        />
                      </div>
                    </MenuItem>
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'Exports' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('Exports')}
                    >
                      <ListItemIcon>
                        <FormControlLabel
                          control={<IOSSwitch sx={{ m: 1 }} onChange={() => setExportsCheck(!exportsCheck)} checked={exportsCheck} />}
                          label=""
                        />
                      </ListItemIcon>
                      Exports
                      ...
                      >
                        <SettingsOutlinedIcon
                          color="primary"
                          style={{
                            opacity: dataExportConfig?.config?.length > 0 ? 1 : 0.3,
                          }}
                        />
                      </div>
                    </MenuItem>
                    ...
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'Data_Insights' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('Data_Insights')}
                    >
                      <ListItemIcon>
                        <FormControlLabel
                          control={<IOSSwitch sx={{ m: 1 }} onChange={() => setInsightsCheck(!insightsCheck)} checked={insightsCheck} />}
                          label=""
                        />
                      </ListItemIcon>
                      Data Insights
                    ...
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'DATA_CHARTING' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('DATA_CHARTING')}
                    >
                      <ListItemIcon>
                        <FormControlLabel
                          control={<IOSSwitch sx={{ m: 1 }} checked={chartsCheck} onChange={() => setChartsCheck(!chartsCheck)} />}
                          label=""
                        />
                      </ListItemIcon>
                      Data Charts
                      ...
                      >
                        <SettingsOutlinedIcon
                          color="primary"
                          style={{
                            opacity: dataChartsConfig?.config?.length > 0 ? 1 : 0.3,
                          }}
                        />
                      </div>
                    </MenuItem>
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'Data_OTA' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('Data_OTA')}
                    >
                      <ListItemIcon>
                        <FormControlLabel control={<IOSSwitch sx={{ m: 1 }} onChange={() => setOtaCheck(!otaCheck)} checked={otaCheck} />} label="" />
                      </ListItemIcon>
                    ...
                    <MenuItem
                      style={{
                        borderBottom: `1px solid ${theme.palette.grey[300]}`,
                        backgroundColor: selectedMenuItem === 'PPT_GEN' ? `${theme.palette.grey[200]}` : 'inherit',
                      }}
                      onClick={() => handleMenuItemClick('PPT_GEN')}
                    >
                      <ListItemIcon>
                        <FormControlLabel control={<IOSSwitch sx={{ m: 1 }} onChange={() => setPptCheck(!pptCheck)} checked={pptCheck} />} label="" />
                      </ListItemIcon>
                      PPT Generator
                      <div
                        style={{
                  ...
                  {selectedMenuItem === 'Data_Cleaning' && (
                    <>
                      ...
                      <Box style={{ padding: '0rem 0rem 1rem 1.5rem' }}>
                        <CustomFormControlLabel
                          control={
                            <Checkbox
                              ...
                              onChange={(e) => handleDataCleaningConfigChange('DATA_CLEANING_STRAIGHT_LINER', 'enabled', e?.target?.checked)}
                            />
                          }
                        ...
                        <Grid container spacing={2} style={{ padding: '0.5rem 1.5rem 1.5rem 1.5rem' }}>
                          <Grid item xs={3}>
                            <label style={{ marginLeft: '5px' }}>Minimum Statement Count</label>
                            <TextField
                              type="number"
                              onKeyPress={handleKeyPress}
                              InputProps={{ inputProps: { min: 0 } }}
                              value={dataCleaningConfig?.config[0]?.fields?.find((f) => f.name === 'statement_count')?.value}
                              onChange={(e) =>
                                handleDataCleaningConfigChange('DATA_CLEANING_STRAIGHT_LINER', 'statement_count', Number(e?.target?.value))
                              }
                            />
                          </Grid>
                          <Grid item xs={3}>
                            <label style={{ marginLeft: '5px' }}>Minimum Options Count</label>
                            <TextField
                              type="number"
                              onKeyPress={handleKeyPress}
                              InputProps={{ inputProps: { min: 0 } }}
                              value={dataCleaningConfig?.config[0]?.fields?.find((f) => f.name === 'option_count')?.value}
                              onChange={(e) =>
                                handleDataCleaningConfigChange('DATA_CLEANING_STRAIGHT_LINER', 'option_count', Number(e?.target?.value))
                              }
                            />
                          </Grid>
                          <Grid item xs={3}>
                            <label style={{ marginLeft: '5px' }}>Threashold</label>
                            <TextField
                              type="number"
                              onKeyPress={handleKeyPress}
                              // InputProps={{ inputProps: { min: 0 } }}
                              value={dataCleaningConfig?.config[0]?.fields?.find((f) => f.name === 'threashold')?.value}
                              onChange={(e) =>
                                handleDataCleaningConfigChange('DATA_CLEANING_STRAIGHT_LINER', 'threashold', parseFloat(e?.target?.value))
                              }
                            />
                          </Grid>
                        </Grid>
                          ...
                          <CustomFormControlLabel
                            control={
                              <Checkbox
                                ...
                                onChange={(e) => handleDataCleaningConfigChange('DATA_CLEANING_SPEEDER', 'enabled', e?.target?.checked)}
                              />
                            }
                            label={'Speeder check'}
                          />
                          ...
                          <Grid container spacing={2} style={{ padding: '0.5rem 1.5rem 1.5rem 1.5rem' }}>
                            <Grid item xs={3}>
                              ...
                              <TextField
                                ...
                                onKeyPress={handleKeyPress}
                                // InputProps={{ inputProps: { min: 0 } }}
                                value={dataCleaningConfig?.config[1]?.fields?.find((f) => f.name === 'loi')?.value}
                                onChange={(e) => handleDataCleaningConfigChange('DATA_CLEANING_SPEEDER', 'loi', parseFloat(e?.target?.value))}
                              />
                            </Grid>
                          </Grid>
                        </Box>
                        ...
                        <CustomFormControlLabel
                          control={
                            <Checkbox
                              ...
                              onChange={(e) => handleDataValidationEnabledChange(e?.target?.checked)}
                            />
                          }
                          label={'Validate programming conditions'}
                        />

                        <Grid container spacing={2} style={{ padding: '0rem 1.5rem 1.5rem 1.5rem' }}>
                          <Grid item xs={4}>
                            <FormControlLabel
                              control={
                                <Checkbox
                                  checked={dataValidationConfig?.config[0]?.fields?.find((f) => f.name === 'condition_check')?.value}
                                  onChange={(e) => handleDataValidationFieldChange('condition_check', e?.target?.checked)}
                                />
                              }
                          ...
                          <Grid item xs={4}>
                            <FormControlLabel
                              control={
                                <Checkbox
                                  checked={dataValidationConfig?.config[0]?.fields?.find((f) => f.name === 'datavalidation_check')?.value}
                                  onChange={(e) => handleDataValidationFieldChange('datavalidation_check', e?.target?.checked)}
                                />
                              }
                  ...
                  {selectedMenuItem === 'Data_Tabulation' && (
                    <>
                      ...
                      >
                        ...
                        <Grid item md={4} xs={12}>
                          <label style={{ marginLeft: '5px' }}>Minimum Sample Size*</label>
                          <TextField
                            size="small"
                        ...
                        <CustomFormControlLabel
                          control={
                            <Checkbox
                              ...
                              onChange={(e) => handleDATAEXPORTSEnabledChange(e?.target?.checked)}
                            />
                          }
                          label={'Output Files'}
                        />
                        ...
                        <Grid container spacing={2} style={{ padding: '0rem 1.5rem 1.5rem 1.5rem' }}>
                          <Grid item xs={3}>
                            <FormControlLabel
                              control={
                                <Checkbox
                                  checked={dataExportConfig?.config[0]?.fields?.find((f) => f.name === 'fixed_width_file_output')?.value}
                                  onChange={(e) => handleDATAEXPORTSFieldChange('fixed_width_file_output', e?.target?.checked)}
                                />
                              }
                          ...
                          <Grid item xs={3}>
                            <FormControlLabel
                              control={
                                <Checkbox
                                  checked={dataExportConfig?.config[0]?.fields?.find((f) => f.name === 'spss_file_output')?.value}
                                  onChange={(e) => handleDATAEXPORTSFieldChange('spss_file_output', e?.target?.checked)}
                                />
                              }
                              label={'SPSS file'}
                            />
                          ...
                          <Grid item xs={3}>
                            <FormControlLabel
                              control={
                                <Checkbox
                                  checked={dataExportConfig?.config[0]?.fields?.find((f) => f.name === 'sav_file_output')?.value}
                                  onChange={(e) => handleDATAEXPORTSFieldChange('sav_file_output', e?.target?.checked)}
                                />
                              }
                              label={'SAV file'}
                            />
                          ...
                          <Grid item xs={3}>
                            <FormControlLabel
                              control={
                                <Checkbox
                                  checked={dataExportConfig?.config[0]?.fields?.find((f) => f.name === 'script_file_output')?.value}
                                  onChange={(e) => handleDATAEXPORTSFieldChange('script_file_output', e?.target?.checked)}
                                />
                              }
                              label={'Script file'}
                            />
                        ...
                        <CustomFormControlLabel
                          control={
                            <Checkbox
                              ...
                              onChange={(e) => handleDataInsightsEnabledChange(e?.target?.checked)}
                            />
                          }
                          label={'Run Insights'}
                        />
                        ...
                        <CustomFormControlLabel
                          control={
                            <Checkbox
                              ...
                              onChange={(e) => handleDataChartsConfigChange('DATA_CHARTING', 'enabled', e?.target?.checked)}
                            />
                          }
                          label={'Data Charting'}
                        />
                  ...
                  {selectedMenuItem === 'Data_OTA' && (
                    <>
                      ...
                      <Box style={{ padding: '0rem 0rem 1rem 1.5rem' }}>
                        <CustomFormControlLabel
                          control={
                            <Checkbox checked={dataOtaConfig?.config[0]?.enabled} onChange={(e) => handleDataOtaEnabledChange(e?.target?.checked)} />
                          }
                  ...
                  {selectedMenuItem === 'PPT_GEN' && (
                    <>
                      ...
                      {/* <Box style={{ padding: "0rem 0rem 1rem 1.5rem" }}>
                        <CustomFormControlLabel
                          control={
                            <Checkbox checked={dataPPTConfig?.config[0]?.enabled} onChange={(e) => handleDataPPTEnabledChange(e?.target?.checked)} />
                          }
                          label={'Run PPT'}
                        />
                      </Box> */}
