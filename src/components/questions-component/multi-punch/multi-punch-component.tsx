...
import { QuestionAnsTypes, QuestionBaseCategoryTypes, QuestionBaseTypes, SinglePunchProps } from './multi-punch-types'
import {
  BoxWrapper,
  ErrorBox,
  MDInput,
  ...
  StyledRowWrapper,
  StyledTypeText,
} from './multi-punch.style'
import Checkbox from '@mui/material/Checkbox'
import { Button, FormControlLabel, IconButton, ImageList, ImageListItem, Radio, Tooltip, Typography } from '@mui/material'
...
import '../../../global.css'
import QuestionBaseCaptureOrder from '@/components/question-base-capture-order'
import { theme } from '@/constants/theme'
import {
  StyledCardQuestionHeadingTitle,
  StyledLabel,
  // StyledLabelBold,
} from '@/components/question-types-base-comp/question-types-base.style'
...
const MultiPunch: React.FC<SinglePunchProps> = ({
  question,
  surveyQuestionId,
  SaveQuestion,
  add,
  ...
  let noOfAnswers = [
    { value: 1, text: 'Min/max count' },
    // { value: 2, text: "Equal to" },
  ]
  ...
  const dataConverter = () => {
    let payload: QuestionBaseTypes | null = {
      ...
      question_type_id: question?.question_type_id ? Number(question?.question_type_id) : 2,
    ...
    let questionBaseCategoryPayload: QuestionBaseCategoryTypes | null = {
      ...
      capture_order: question?.capture_order ? question?.capture_order : false,
      no_of_answers: question?.no_of_answers ? question?.no_of_answers : '',
      no_of_answers_min: question?.no_of_answers_min ? question?.no_of_answers_min : '',
      no_of_answers_max: question?.no_of_answers_max ? question?.no_of_answers_max : '',
      no_of_answers_count: question?.no_of_answers_count ? question?.no_of_answers_count : '',

      // dummy multi-selct
      precision_value: question?.precision_value ? question?.precision_value : '',
      upper_limit: question?.upper_limit === null || question?.upper_limit === '' ? '' : Number(question?.upper_limit),
      lower_limit: question?.lower_limit === null || question?.upper_limit === '' ? '' : Number(question?.lower_limit),

      no_of_columns: question?.no_of_columns ? question?.no_of_columns : '',
      // auto_sum: question?.auto_sum ? question?.auto_sum : "",
    }
  ...
  // console.log("multiquestionTypeListquestionTypeList", questionBase, questionBaseCategory, questionAns)
  ...
  return (
    <>
      ...
      <BoxWrapper>
        ...
        {isEdit ? (
          <StyledRowGapWrapper sx={{ padding: '0rem', display: surveyQuestionId ? '' : 'none' }}>
            <Stack
              direction="row"
              alignItems="center"
        ...
        {isEdit ? (
          <StyledContentBox sx={{ display: surveyQuestionId ? '' : 'none' }}>
                              ...
                              <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                ...
                                /> */}
                                  {questionBaseCategory?.show_codes ? (
                                    <Stack className="stack-question_answer_code" sx={{ width: '10% !important' }}>
                                      <Tooltip
                                        title={
                                  ...
                                  <Stack
                                    ...
                                    sx={{
                                      width: questionBaseCategory?.show_codes ? '80% !important' : '90% !important',
                                    }}
                                  >
                                    {/* {index === 0 && (
                                    ...
                                    <MDInput
                                      ...
                                      className="input-question_answer_text-multi-select"
                                      variant="standard"
                                      placeholder="Text"
            ...
            <StyledEditOptionsBox className="answer-add-container" marginBottom={''}>
              {/* <FormControlLabel
            ...
            <ImageList sx={{ width: '100%', gridTemplateColumns: questionAns.some((item) => item?.concept?.file_url) ? 'repeat(2, 1fr) !important' : 'repeat(1, 1fr) !important', }}>
              {questionAns &&
                ...
                questionAns
                  ?.filter((answer) => answer.is_active)
                  .map((answer, ind) => (
                    // <StyledCardAnswerText className="form-control-answer-label" style={{ alignItems: 'start' }}>
        ...
        {isEdit ? (
          <StyledContentBox className="configuration-box" sx={{ marginBottom: '0rem' }}>
            ...
            >
              <StyledGridContainer container spacing={1} sx={{ padding: '0px' }}>
                <Grid item xs={11.8}>
                  <Select
                ...
                <Grid item xs={11.8}>
                  <Select
                    className="base-comp .MuiInputBase-input"
                    value={questionBaseCategory?.precision_value?.toString()}
                    items={noOfAnswers}
                    onChange={(e) => {
                      let payload: any = { ...questionBaseCategory }
                ...
                {questionBaseCategory?.precision_value && questionBaseCategory?.precision_value == 1 ? (
                  <>
                    <Grid item xs={6}>
                      <MDInput
                        label="Min"
                      ...
                      />
                      {questionBaseCategory?.lower_limit && Number(questionBaseCategory?.lower_limit) <= 0 ? (
                        <ErrorBox>Count should more than 0</ErrorBox>
                      ) : questionBaseCategory?.upper_limit && !questionBaseCategory?.lower_limit ? (
                        <ErrorBox>Please enter minimum count</ErrorBox>
                      ) : (
                        ''
                      )}
                    </Grid>
                    <Grid item xs={6}>
                      ...
                      />
                      {questionAns.length > 0 &&
                        questionBaseCategory?.upper_limit &&
                        Number(questionBaseCategory?.upper_limit) > Number(questionAns.length) ? (
                        <ErrorBox>Count should not more than Answers</ErrorBox>
                      ) : questionBaseCategory?.lower_limit &&
                        questionBaseCategory?.upper_limit &&
                        Number(questionBaseCategory?.lower_limit) >= Number(questionBaseCategory?.upper_limit) ? (
                        <ErrorBox>Max Count shound more than min</ErrorBox>
                      ) : (
                        ''
                      )}
                    </Grid>
                  </>
                ) : questionBaseCategory?.precision_value && questionBaseCategory?.precision_value == 2 ? (
                  <Grid item xs={12}>
                    <MDInput
                      label="Count"
                      className="base-comp-no_of_answers_min .MuiInputBase-input"
                    ...
                    />
                    {questionBaseCategory?.upper_limit && Number(questionBaseCategory?.upper_limit) > Number(questionAns.length) ? (
                      <ErrorBox>Count should not more than Answers</ErrorBox>
                    ) : questionBaseCategory?.upper_limit && Number(questionBaseCategory?.upper_limit) <= 0 ? (
                      <ErrorBox>Count should more than 0</ErrorBox>
                    ) : questionBaseCategory?.precision_value && Number(questionBaseCategory?.upper_limit) <= 0 ? (
                      <ErrorBox>Please enter some count</ErrorBox>
                    ) : (
                      ''
                    )}
                  </Grid>
            ...
            <Grid
              ...
              style={{
                ...
                paddingLeft: '0.5rem',
                paddingBottom: '1rem',
                display: collapseConf ? 'none' : 'flex',
              }}
            >
              <Grid xs={6} style={{ marginBottom: '-0.5rem' }}>
              ...
              <Grid xs={6} style={{ paddingLeft: '0.8rem' }}>
                <QuestionBaseCaptureOrder setQuestionBaseCategory={setQuestionBaseCategory} questionBaseCategory={questionBaseCategory} />
              </Grid>
              <Grid xs={6} style={{ paddingLeft: '0.9rem' }}>
                <FormControlLabel
                  // sx={{ marginLeft: "5px" }}
                  control={
                    <Switch
                  ...
                  label={<StyledLabel sx={{ paddingTop: '0rem', marginLeft: '10px' }}>Can have Quota</StyledLabel>}
                />
              </Grid>
            </Grid>
            <StyledAnswersDivider
            ...
            {/* <Button
              ...
              onClick={() => {
                if (add) {
                  ...
                  dataConverter();
                  setIsEdit(false);
                  setEditFalse && setEditFalse(false);
                }
              }}
            >
              Cancel
