...
import { QuestionAnsTypes, QuestionBaseTypes, GridSinglePunchProps, PromptAnsTypes, QuestionBaseCategoryTypes } from './grid-single-punch-types'
import {
  BoxWrapper,
  ErrorBox,
  MDInput,
...
import { StyledAnswersDivider } from '../single-punch/single-punch.style'
...
import '../../../global.css'
import GridSinglePunchViewTable from './grid-single-punch-view-table'
...
const GridSinglePunch: React.FC<GridSinglePunchProps> = ({
  question,
  surveyQuestionId,
  SaveQuestion,
  add,
  ...
  const dataConverter = () => {
    let payload: QuestionBaseTypes | null = {
      ...
      question_type_id: question?.question_type_id ? Number(question?.question_type_id) : 12,
  ...
  useEffect(() => {
    ...
    let checkPromptCode = promptAns && promptAns?.length > 0 && promptAns?.some((answer) => answer.prompt_code === '')
    let uniquePromptCodeCheck = promptAns && promptAns?.length > 0 && new Set(promptAns?.map((a) => a.prompt_code)).size !== promptAns.length

    // let promptAnsCheck = promptAns && promptAns?.length > 0 && promptAns?.some((answer) => answer.prompt_text === '')
    if (
      surveyQuestionId
  ...
  useEffect(() => {
    if (!isEdit && questionPreview) {
      ...
      setPromptAns(questionPreview?.promptAns)
    }
  }, [questionPreview])
  ...
  // console.log(questionAns, promptAns, "promptAnspromptAns", questionPreview, question, promptAnswersSwap, deletedPromptAns, deletedQuestionAns)
  ...
  return (
    <>
      ...
      <BoxWrapper>
        ...
        {isEdit ? (
          <StyledContentBox
            // sx={{ display: collapsePromts ? "none" : "" }}
          ...
          <StyledContentBox className="answer-add-container-view-mode">
            <StyledEditOptionsBox sx={{ padding: '0px' }}>
              <GridSinglePunchViewTable promptAns={promptAns} questionAns={questionAns} questionTheme={questionTheme} />
            </StyledEditOptionsBox>
          </StyledContentBox>
        )}
                              ...
                              <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                ...
                                /> */}
                                  ...
                                  <Stack
                                    className="stack-question_answer_text"
                                    sx={{
                                      width: questionBaseCategory?.show_codes ? '100% !important' : '100% !important',
                                    }}
                                  >
                                    {/* {index === 0 && (
                                    <StyledLabel sx={{ padding: "0px" }}>
            ...
            <StyledEditOptionsBox className="answer-add-container" style={{ padding: '0px' }}>
              ...
              <MDInput
                ...
                onFocus={() => {
                  ...
                  payload.push({
                    question_answer_id: '',
                    // question_answer_id: questionAns.length,
        ...
        {isEdit ? (
          <StyledContentBox className="configuration-box" style={{ marginBottom: '-0.5rem' }}>
            ...
            <StyledEditOptionsBox style={{ padding: '0px' }} sx={{ display: collapseConf ? 'none' : '' }}>
              <StyledGridContainer
                container
                spacing={1}
                xs={12}
