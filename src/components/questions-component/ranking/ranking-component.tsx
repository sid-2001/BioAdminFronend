...
import { QuestionAnsTypes, QuestionBaseCategoryTypes, QuestionBaseTypes, SinglePunchProps } from './ranking-types'
import {
  BoxWrapper,
  ErrorBox,
  MDInput,
...
import { IconButton, Box, Button, Typography, Tooltip, ImageList, ImageListItem } from '@mui/material'
...
const Ranking: React.FC<SinglePunchProps> = ({
  question,
  surveyQuestionId,
  SaveQuestion,
  add,
  ...
  const handleInputClick = (index: number) => {
    if (clickedIndices.includes(index)) {
      const newClickedIndices = clickedIndices.filter((i) => i !== index)
      setClickedIndices(newClickedIndices)
    } else if (clickedIndices.length < questionAns.length) {
  ...
  const dataConverter = () => {
    let payload: QuestionBaseTypes | null = {
      ...
      question_type_id: question?.question_type_id ? Number(question?.question_type_id) : 9,
    ...
    let questionBaseCategoryPayload: QuestionBaseCategoryTypes | null = {
      answer_sorting_order: question?.answer_sorting_order ? question?.answer_sorting_order : 0,
      ...
      precision_value: question?.precision_value ? question?.precision_value : 1,

      // capture_order: question?.capture_order ? question?.capture_order : false,
      no_of_answers: question?.no_of_answers ? question?.no_of_answers : 0,
      no_of_answers_min: question?.no_of_answers_min ? question?.no_of_answers_min : 0,
      no_of_answers_max: question?.no_of_answers_max ? question?.no_of_answers_max : 0,
      no_of_answers_count: question?.no_of_answers_count ? question?.no_of_answers_count : 0,
  ...
  return (
    <>
      ...
      <BoxWrapper>
        ...
        {isEdit ? (
          <StyledRowGapWrapper sx={{ padding: '0px 0px 0px 0px', display: surveyQuestionId ? '' : 'none' }}>
            <Stack
              direction="row"
              alignItems="center"
        ...
        {isEdit ? (
          <StyledContentBox
            // sx={{ display: collapseAns ? "none" : "" }}
            sx={{ display: surveyQuestionId ? '' : 'none' }}
          >
            {/* --- */}
                              ...
                              <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                ...
                                /> */}
                                  {questionBaseCategory?.show_codes ? (
                                    <Stack className="stack-question_answer_code" sx={{ width: '30% !important' }}>
                                      <Tooltip
                                        title={
                  ...
                  .map((answer, index) => (
                    <>
                      <ImageListItem key={index} style={{
                        ...
                        {/* <StyledCardAnswerText className="form-control-answer-div-input-box" key={index}> */}
                        <MDInput
                          value={clickedIndices.includes(index) ? clickedIndices.indexOf(index) + 1 : ''}
          ...
          <StyledContentBox>
            ...
            <StyledEditOptionsBox sx={{ display: collapseConf ? 'none' : '' }}>
              <StyledGridContainer container spacing={1}>
                <Grid item xs={12} style={{ marginBottom: '0.3rem' }}>
                  <Select
                    className="configuration-box-select"
                    value={(questionBaseCategory?.answer_sorting_order && Number(questionBaseCategory?.answer_sorting_order)) || 0}
            ...
            <StyledEditOptionsBox style={{ display: collapseConf ? 'none' : '' }}>
              <StyledGridContainer container spacing={2} xs={12} className="configuration-box-sub-cont" style={{ paddingLeft: '1.5rem' }}>
                <Grid xs={12}>
