...
const ProjectPipelinesContainer = () => {
    ...
    const handleDetailsOptionsClose = (
        ...
        event.stopPropagation();
        navigate(`/projects/${projectId}/survey/${surveyId}/data/pipelines/${request?.id}/update`, { state: request?.is_active })
        setDownloadEl(null);
    };
    ...
    async function pipelineRun(pipeline_id: number) {
        setLoading(true);
    ...
    async function getAllPipelinesData() {
        ...
        try {
            const data = await projectDataService.GetAllPipelines(Number(projectId), Number(surveyId));
            setRequests(data);
        } catch (error) {
            enqueueSnackbar(
                <Typography variant="body1">Fetching requests failed</Typography>,
                    ...
                    <GridContainerProject style={{ marginBottom: "2rem" }}>
                        ...
                        >
                            <AddCard
                                ...
                                handleClick={() => navigate(`/projects/${projectId}/survey/${surveyId}/data/pipelines/new`)}
                                // handleClick={() => setViewMode(true)}
                                height={"170px"}
                                width={"300px"}
                            />
                        </Box>
